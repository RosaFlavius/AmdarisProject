...
export const newClothesSchema = yup.object().shape({
  ...
  gender: yup.number().required("Please select one gender").min(1).max(3),
  size: yup.number().required("Please select one size").min(1).max(6),
});
